...
program.  During the run, it outputs the text file `fife.log` with a
...
WARNING: `fife.log` can be large, depending on how long you run the
...
uses the Python tool in `Tools/Log_to_CSV` to process `fife.log` into
`fife.log.csv` which is a standard "Comma-Separated Values" file
